      ...
      <div class="col-md-12">
        <form>
            <div class="form-group form-md-line-input form-md-floating-label" :class="{'has-error': errors.has('name')}">
              ...
              <label for="name">机场名字</label>
              <span class="help-block">
                <i class="fa fa-check" v-show="!errors.has('name')"></i> 请输入一个机场的名字...
              </span>
            </div>
            <div class="form-group form-md-line-input form-md-floating-label" :class="{'has-error': errors.has('description')}">
              ...
              <span class="help-block">
                <i class="fa fa-check" v-show="!errors.has('description')"></i> 对机场做一些描述...
              </span>
            </div>
        </form>
      </div>
  ...
  export default {
    ...
    watch: {
      ...
      show (val) {
        ...
        if (val) {
          this.form.name = this.airport.Name
          this.form.description = this.airport.Description
          this.form.id = this.airport.ID
          this.errors.clear()
        }
      }
    },
